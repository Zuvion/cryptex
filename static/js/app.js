...
async function updateTradeStats() {
  try {
    ...
    const wEl = document.getElementById('statsWins');
    const lEl = document.getElementById('statsLosses');
    const tEl = document.getElementById('statsTotal');
    if (wEl) wEl.textContent = s.wins_count || 0;
    if (lEl) lEl.textContent = s.losses_count || 0;
    if (tEl) tEl.textContent = s.total_trades || 0;
  } catch(e) {}
}
  ...
  pageItems.forEach(h => {
    ...
    const method = (() => {
      if (h.type === 'deposit' && h.details) {
        ...
        if (h.details.method === 'cryptobot') return '🤖 CryptoBot';
        ...
        if (h.details.invoice_id && h.details.invoice_id.startsWith('cryptobot_')) return '🤖 CryptoBot';
      }
          ...
          <div class="section-content" id="profileContent">
            <div style="display:flex;flex-direction:column;gap:6px">
              <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:rgba(255,255,255,0.04);border-radius:10px;border:1px solid rgba(255,255,255,0.06)">
                <span style="color:#7B8CA2;font-size:12px">ID</span>
                <span style="color:#EAECEF;font-size:13px;font-family:monospace;font-weight:600">${stats.telegram_id || TG_USER?.id || '—'}</span>
              </div>
              <div id="tradeStatsRow" style="display:flex;gap:6px">
                <div style="flex:1;text-align:center;padding:10px 6px;background:rgba(0,230,118,0.06);border-radius:10px;border:1px solid rgba(0,230,118,0.15)">
                  <div style="color:#7B8CA2;font-size:10px;margin-bottom:4px">${t('profile.wins')}</div>
                  <div id="statsWins" style="color:#00E676;font-size:16px;font-weight:700">${stats.wins_count || 0}</div>
                </div>
                <div style="flex:1;text-align:center;padding:10px 6px;background:rgba(255,82,82,0.06);border-radius:10px;border:1px solid rgba(255,82,82,0.15)">
                  <div style="color:#7B8CA2;font-size:10px;margin-bottom:4px">${t('profile.losses')}</div>
                  <div id="statsLosses" style="color:#FF5252;font-size:16px;font-weight:700">${stats.losses_count || 0}</div>
                </div>
                <div style="flex:1;text-align:center;padding:10px 6px;background:rgba(255,255,255,0.04);border-radius:10px;border:1px solid rgba(255,255,255,0.06)">
                  <div style="color:#7B8CA2;font-size:10px;margin-bottom:4px">${t('profile.total_trades')}</div>
                  <div id="statsTotal" style="color:#EAECEF;font-size:16px;font-weight:700">${stats.total_trades || 0}</div>
                </div>
              </div>
            </div>
          </div>
...
async function openDeposit(){
  ...
  const cryptoBotCurrencies = ['USDT', 'TON', 'BTC', 'ETH', 'BNB', 'LTC', 'TRX', 'USDC', 'NOT', 'DOGS'];
  ...
  function showDepositMethodSelection() {
    ...
    <div class="container" style="padding:16px">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ...
        <div id="methodCryptoBot" style="background:#131A2A;border-radius:6px;padding:18px 16px;display:flex;align-items:center;justify-content:space-between;cursor:pointer;border:1px solid transparent;transition:all 0.2s">
          <div style="display:flex;align-items:center;gap:14px">
            <div style="width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,#0098EA,#4FC3F7);display:flex;align-items:center;justify-content:center;font-size:22px">🤖</div>
            <div>
              <div style="font-weight:600;font-size:16px;color:#fff">CryptoBot</div>
              <div style="font-size:12px;color:#7B8CA2">${t('deposit.method_cryptobot')}</div>
            </div>
          </div>
          <span style="color:#7B8CA2;font-size:18px">›</span>
        </div>
    ...
    const cb = document.getElementById('methodCryptoBot');
    ...
    cb.onclick = () => showCryptoBotCurrencySelection();
  ...
  function showCryptoBotCurrencySelection() {
    const titleText = t('deposit.select_currency');
    cont.innerHTML = `
    <div class="container" style="padding:16px">
      <button class="btn" id="backCB" style="background:transparent;border:none;color:#fff;font-size:16px;padding:8px 0;margin-bottom:16px">← ${t('btn.back')}</button>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:20px">
        <div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,#0098EA,#4FC3F7);display:flex;align-items:center;justify-content:center;font-size:16px">🤖</div>
        <div class="section-title" style="font-size:20px;font-weight:700;margin:0">${titleText}</div>
      </div>
      <div id="cbCoinList" style="display:flex;flex-direction:column;gap:8px">
        ${cryptoBotCurrencies.map(sym => {
          const logo = cryptoLogos[sym] || '';
          const logoHTML = logo ? `<img src="${logo}" style="width:40px;height:40px;border-radius:50%" onerror="this.style.display='none'"/>` : `<div style="width:40px;height:40px;border-radius:50%;background:#333;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:700;color:#4FC3F7">${sym[0]}</div>`;
          return `<div class="cb-coin-card" data-sym="${sym}" style="background:#131A2A;border-radius:6px;padding:14px 16px;display:flex;align-items:center;justify-content:space-between;cursor:pointer;border:1px solid transparent;transition:all 0.2s">
            <div style="display:flex;align-items:center;gap:12px">
              ${logoHTML}
              <div style="font-weight:600;font-size:15px;color:#fff">${sym}</div>
            </div>
    ...
    document.getElementById('backCB').onclick = showDepositMethodSelection;

    document.querySelectorAll('.cb-coin-card').forEach(card => {
      card.onclick = () => showCryptoBotAmountInput(card.dataset.sym);
      card.onmouseenter = () => { card.style.borderColor = '#E040FB'; card.style.background = '#252525'; };
      card.onmouseleave = () => { card.style.borderColor = 'transparent'; card.style.background = '#131A2A'; };
    });
  }
  ...
  function showCryptoBotAmountInput(currency) {
    ...
    cont.innerHTML = `
    <div class="container" style="padding:16px">
      <button class="btn" id="backCBCur" style="background:transparent;border:none;color:#fff;font-size:16px;padding:8px 0;margin-bottom:16px">← ${t('btn.back')}</button>
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px">
        ${logo ? `<img src="${logo}" style="width:32px;height:32px;border-radius:50%"/>` : ''}
        <div class="section-title" style="font-size:20px;font-weight:700;margin:0">${currency}</div>
        <span style="font-size:11px;color:#4FC3F7;background:rgba(0,152,234,0.2);padding:4px 10px;border-radius:12px">CryptoBot</span>
      </div>
      <div style="color:#7B8CA2;font-size:13px;margin-bottom:20px">${t('deposit.enter_amount_in') + ' ' + currency}</div>
    ...
    document.getElementById('backCBCur').onclick = () => showCryptoBotCurrencySelection();

    const customInput = document.getElementById('cbCustomAmount');

    document.querySelectorAll('.cb-preset-btn').forEach(btn => {
      btn.onclick = () => {
        customInput.value = btn.dataset.amount;
        document.querySelectorAll('.cb-preset-btn').forEach(b => { b.style.borderColor = '#3A3A3A'; b.style.background = '#2A2A2A'; });
        btn.style.borderColor = '#E040FB'; btn.style.background = 'rgba(224,64,251,0.12)';
      };
      btn.onmouseenter = () => { if (btn.style.borderColor !== 'rgb(224, 64, 251)') btn.style.background = '#333'; };
      btn.onmouseleave = () => { if (btn.style.borderColor !== 'rgb(224, 64, 251)') btn.style.background = '#2A2A2A'; };
    ...
    customInput.oninput = () => {
      const val = Number(customInput.value || 0);
      document.querySelectorAll('.cb-preset-btn').forEach(b => {
        if (Number(b.dataset.amount) === val) { b.style.borderColor = '#E040FB'; b.style.background = 'rgba(224,64,251,0.12)'; }
        else { b.style.borderColor = '#3A3A3A'; b.style.background = '#2A2A2A'; }
      });
    };

    document.getElementById('cbSubmit').onclick = async () => {
      const amount = Number(customInput.value || 0);
      if (!amount || (minAmount && amount < minAmount)) { toast(`${t('deposit.min_amount_toast')}: ${minAmount || '> 0'} ${currency}`); return; }
      await createCryptoBotInvoice(amount, currency);
    };
  }

  async function createCryptoBotInvoice(amount, currency) {
    toast(t('deposit.creating_invoice'));
    try {
      const res = await apiFetch('/api/deposit/cryptobot/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount, currency })
      });
  ...
  function showCryptoBotPaymentScreen(payData) {
    const { invoice_id, bot_link, amount, currency } = payData;
    const waitText = t('deposit.waiting');
    const descText = t('deposit.pay_via_cryptobot');
    const amountLabel = t('history.amount');
    const methodLabel = t('history.method');
    const paidText = t('deposit.status_paid');
    const cancelText = t('btn.cancel');
    const payBtnText = t('deposit.pay_cryptobot');

    cont.innerHTML = `
    <div class="container" style="padding:16px">
      <div style="text-align:center;padding:24px 0">
        <div id="cbPaySpinner" style="margin-bottom:16px">
          <div style="width:60px;height:60px;margin:0 auto;border:3px solid rgba(255,255,255,0.08);border-top-color:#4FC3F7;border-radius:50%;animation:spin 1s linear infinite"></div>
        </div>
        <div id="cbPayIcon" style="display:none;font-size:56px;margin-bottom:16px">✅</div>
        <h2 id="cbPayTitle" style="color:#EAECEF;margin-bottom:6px;font-size:18px">${waitText}</h2>
        <p style="color:#7B8CA2;font-size:13px;margin-bottom:4px">${descText}</p>
      </div>
      ...
      <div style="background:#131A2A;border-radius:8px;padding:16px;margin-bottom:16px">
        ...
        <div style="display:flex;justify-content:space-between">
          <span style="color:#7B8CA2;font-size:13px">${methodLabel}</span>
          <span style="color:#4FC3F7;font-size:14px;font-weight:600">🤖 CryptoBot</span>
        </div>
      </div>
    ...
    let cbPollCount = 0;
    const cbPollInterval = setInterval(async () => {
      cbPollCount++;
      if (cbPollCount > 720) { clearInterval(cbPollInterval); return; }
      try {
        const res = await apiFetch(`/api/deposit/cryptobot/check?invoice_id=${invoice_id}`);
        const data = await res.json();
        if (data.paid) {
          clearInterval(cbPollInterval);
          document.getElementById('cbPaySpinner').style.display = 'none';
          document.getElementById('cbPayIcon').style.display = 'block';
          document.getElementById('cbPayTitle').textContent = paidText;
          document.getElementById('cbPayTitle').style.color = '#00E676';
          document.getElementById('cbPayStatus').textContent = paidText;
          document.getElementById('cbPayStatus').style.color = '#00E676';
          toast(t('deposit.credited_toast'));
          setTimeout(() => renderAssets(), 2500);
        } else if (data.status) {
          const sMap = { 'active': t('deposit.status_waiting'), 'expired': t('deposit.status_expired') };
          document.getElementById('cbPayStatus').textContent = sMap[data.status] || data.status;
        }
      } catch(e) { console.error('CryptoBot poll error', e); }
    }, 5000);
  }
...
async function openPair(pair, displayName = null){
  ...
  async function loadChartData() {
    try {
      ...
      // Во время симуляции сделки не перезаписываем серию реальными свечами —
      // иначе микро-свечи стираются и график "прыгает" (визуальный артефакт)
      if (!window._simBusy) {
        if (isFirstChartLoad) {
  ...
  // ================================================================
  //  ДВИЖОК ЦЕНЫ: 3 слоя
  //  1. Data Layer       — базовый рынок (реальная цена + естественный шум)
  //  2. Simulation Layer — drift сделки (noise + drift + easing)
  //  3. Render Layer     — requestAnimationFrame + anti-spike
  // ================================================================
  ...
  // ---------- Слой 1: Data Layer (базовый рынок) ----------
  const feed = {
    realPrice: 0,        // последняя цена с сервера (цель)
    basePrice: 0,        // сглаженная базовая цена (lerp к realPrice)
    noise: 0,            // mean-reverting микрошум → естественные откаты
    microTrend: 0,       // локальный микро-тренд (изм. цены в сек)
    microTrendUntil: 0,  // когда сменить микро-тренд
  };

  const priceFetchTimer = setInterval(async () => {
    try {
      ...
      if (!p || p <= 0) return;
      feed.realPrice = p;
      if (!feed.basePrice) feed.basePrice = p;

      const changeEl = document.getElementById('pairChange24h');
      if (changeEl && typeof priceData === 'object' && priceData.change_24h !== undefined) {
        const ch = priceData.change_24h;
        changeEl.textContent = (ch >= 0 ? '+' : '') + ch.toFixed(2) + '%';
        changeEl.style.color = ch >= 0 ? '#00E676' : '#FF5252';
      }
    } catch(e) {}
  }, 1000);

  // ---------- Слой 2: Simulation Layer (drift сделки) ----------
  const sim = {
    state: 'idle',          // idle | active | release
    trade: null,
    drift: 0,               // текущее применённое смещение
    driftTarget: 0,
    pullbackPhase: Math.random() * Math.PI * 2,
    tradeStartMs: 0,
    tradeTotalMs: 60000,
    releaseFrom: 0,
    releaseStart: 0,
    releaseDuration: 2000,  // плавный возврат к рынку 1.5–2.5 сек
  };
  window._simBusy = false;

  function simUpdate(dt) {
    const nowMs = Date.now();

    // --- базовый рынок: плавное следование за реальной ценой ---
    feed.basePrice = lerp(feed.basePrice, feed.realPrice, 1 - Math.exp(-dt * 1.2));

    // микро-тренды: каждые 2.5–8.5 сек направление слегка меняется
    if (nowMs > feed.microTrendUntil) {
      feed.microTrend = (Math.random() - 0.5) * feed.basePrice * 0.00018;
      feed.microTrendUntil = nowMs + 2500 + Math.random() * 6000;
    }
    // шум: тренд + джиттер, mean-reversion создаёт естественные откаты
    const jitter = (Math.random() - 0.5) * feed.basePrice * 0.00035 * Math.sqrt(dt);
    feed.noise += feed.microTrend * dt + jitter;
    feed.noise -= feed.noise * Math.min(1, dt * 0.35);
    const noiseCap = feed.basePrice * 0.0012;
    feed.noise = clampVal(feed.noise, -noiseCap, noiseCap);

    // --- вход в режим сделки: бесшовно, drift стартует с текущего значения ---
    if (activeTradeForPair && sim.state !== 'active') {
      const total = activeTradeForPair.duration_sec || 60;
      const left = activeTradeForPair.time_left_sec != null ? activeTradeForPair.time_left_sec : total;
      sim.state = 'active';
      sim.trade = activeTradeForPair;
      sim.tradeTotalMs = total * 1000;
      sim.tradeStartMs = nowMs - (total - left) * 1000;
      tradeMicroCandleInterval = getTradeCandelInterval(total);
      microCandleElapsedMs = 0;
    }

    if (sim.state === 'active') {
      if (!activeTradeForPair) {
        // сделка закрылась — плавный возврат к рынку (easeOutCubic)
        sim.state = 'release';
        sim.releaseFrom = sim.drift;
        sim.releaseStart = nowMs;
        sim.releaseDuration = 1500 + Math.random() * 1000;
      } else {
        sim.trade = activeTradeForPair;
        const progress = clampVal((nowMs - sim.tradeStartMs) / sim.tradeTotalMs, 0, 1);
        const serverTrend = sim.trade._t || 1;
        const sideDir = sim.trade.side === 'buy' ? 1 : -1;
        const direction = sideDir * serverTrend;

        // easing: старт ≈ чистый рынок → середина умеренно → конец выраженно
        const ease = easeInOutCubic(progress);

        // волна откатов, ослабевает к концу сделки
        sim.pullbackPhase += dt * (0.9 + 0.3 * Math.sin(nowMs / 3700));
        const pullbackDepth = 0.4 * (1 - progress * 0.7);
        const pullback = 1 - pullbackDepth * (0.5 + 0.5 * Math.sin(sim.pullbackPhase));

        const maxShift = feed.basePrice * 0.0012;
        sim.driftTarget = direction * maxShift * ease * pullback;
        // drift подтягивается к цели через lerp — без скачков
        sim.drift = lerp(sim.drift, sim.driftTarget, 1 - Math.exp(-dt * 1.6));
      }
    } else if (sim.state === 'release') {
      const k = (nowMs - sim.releaseStart) / sim.releaseDuration;
      if (k >= 1) {
        sim.drift = 0;
        sim.state = 'idle';
        sim.trade = null;
      } else {
        sim.drift = sim.releaseFrom * (1 - easeOutCubic(k));
      }
    }

    window._simBusy = sim.state !== 'idle';
  }

  // ---------- Слой 3: Render Layer (rAF + anti-spike) ----------
  ...
  function renderStep(renderDtMs) {
    const raw = feed.basePrice + feed.noise + sim.drift;

    // anti-spike: жёсткий лимит изменения цены за один тик рендера
    let display = raw;
    if (lastDisplayPrice > 0) {
      const maxDelta = feed.basePrice * 0.0005;
      display = clampVal(raw, lastDisplayPrice - maxDelta, lastDisplayPrice + maxDelta);
    }
    lastDisplayPrice = display;

    const priceEl = document.getElementById('pairCurrentPrice');
    if (priceEl) {
      priceEl.textContent = display < 1 ? display.toFixed(6) : Number(display).toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2});
    }

    const nowSec = Math.floor(Date.now() / 1000);
    const inSimMode = sim.state !== 'idle';

    if (inSimMode) {
      // микро-свечи во время сделки и во время плавного возврата
      microCandleElapsedMs += renderDtMs;
      if (!lastCandleData) {
        lastCandleData = { time: nowSec, open: display, high: display, low: display, close: display };
        candleSeries.update(lastCandleData);
      } else if (microCandleElapsedMs >= tradeMicroCandleInterval * 1000) {
        microCandleElapsedMs = 0;
        const prevClose = lastCandleData.close;
        lastCandleData = {
          time: lastCandleData.time + tradeMicroCandleInterval,
          open: prevClose,
          high: Math.max(prevClose, display),
          low: Math.min(prevClose, display),
          close: display,
        };
        candleSeries.update(lastCandleData);
        chart.timeScale().scrollToPosition(2, false);
      } else {
        lastCandleData.high = Math.max(lastCandleData.high, display);
        lastCandleData.low = Math.min(lastCandleData.low, display);
        lastCandleData.close = display;
        candleSeries.update({ ...lastCandleData });
      }
    } else if (lastCandleData) {
      const bucket = getCandleStartTime(nowSec);
      if (bucket > lastCandleData.time) {
        lastCandleData = { time: bucket, open: display, high: display, low: display, close: display };
        candleSeries.update(lastCandleData);
        chart.timeScale().scrollToPosition(2, false);
      } else {
        lastCandleData.high = Math.max(lastCandleData.high, display);
  ...
  function engineLoop(ts) {
    // страница закрыта — останавливаем движок и чистим таймеры
    if (!document.getElementById('price_chart')) {
      stopPriceEngine();
      return;
    }
    rafId = requestAnimationFrame(engineLoop);

    if (!feed.basePrice) return; // ждём первую цену с сервера
    if (!lastFrameTs) { lastFrameTs = ts; return; }
    let dt = (ts - lastFrameTs) / 1000;
    lastFrameTs = ts;
    if (dt <= 0) return;
    dt = Math.min(dt, 0.25); // защита от фоновой вкладки

    simUpdate(dt);

    if (ts - lastRenderTs >= RENDER_INTERVAL_MS) {
      try { renderStep(lastRenderTs ? ts - lastRenderTs : RENDER_INTERVAL_MS); } catch(e) {}
      lastRenderTs = ts;
    }
  }
  rafId = requestAnimationFrame(engineLoop);

  // очистка движка при выходе со страницы пары
  {
    const backBtn = document.getElementById('backTrade');
    const prevBackHandler = backBtn.onclick;
    backBtn.onclick = () => {
      stopPriceEngine();
      if (prevBackHandler) prevBackHandler();
    };
  }
